...
/**
 * Automated NPM version update and publish script for react-native-advanced-flatlist
 * 
 * This script provides a comprehensive solution for:
 * - Interactive version selection (when no args provided)
 * - Legacy support for direct version type (patch/minor/major)
 ...
 *   npm run release patch              # Direct patch release (legacy)
...
class BuildAndTest {
  ...
  static async buildProject() {
    ...
    if (config.dryRun) {
      logger.success('Project built successfully (dry run mode)');
    } else {
  ...
  static async runTests() {
    ...
    if (packageInfo.scripts.test && packageInfo.scripts.test.includes('no test specified')) {
      logger.warning('No tests specified, skipping test step');
      return;
    }
  ...
  static async verifyPackage() {
    ...
    const result = Utils.executeCommand('npm run publish:check');
    if (!result.success) {
...
async function legacyRelease(versionType) {
  logger.step(`Starting legacy release process for ${versionType} version`);
  
  try {
    // Git status check
    const gitResult = Utils.executeCommand('git status --porcelain', { silent: true });
    if (gitResult.stdout.trim()) {
      throw new Error('Working directory is not clean. Please commit or stash your changes first.');
    }
    
    // Build project
    Utils.executeCommand('npm run clean');
    Utils.executeCommand('npm run build');
    
    // Check package
    Utils.executeCommand('npm run publish:check');
    
    // Update version and publish
    const versionResult = Utils.executeCommand(`npm version ${versionType}`, { silent: true });
    const newVersion = versionResult.stdout.match(/v(\d+\.\d+\.\d+)/)?.[1];
    
    Utils.executeCommand('npm publish');
    
    // Push to git
    Utils.executeCommand('git push');
    Utils.executeCommand('git push --tags');
    
    logger.success(`🎉 Legacy release completed successfully!`);
    console.log(`📦 Package v${newVersion} is now available on npm`);
    
  } catch (error) {
    logger.error(`Legacy release failed: ${error.message}`);
    process.exit(1);
  }
}
...
function parseArguments() {
  ...
  // Check for legacy usage first (npm run release patch/minor/major)
  if (args.length === 1 && ['patch', 'minor', 'major'].includes(args[0])) {
    return { legacy: true, versionType: args[0] };
  }
...
${colors.cyan}Usage:${colors.reset}
  npm run release                         # Interactive release
  npm run release patch|minor|major       # Legacy direct release
  node scripts/release.js [options]       # Advanced release
...
${colors.cyan}Examples:${colors.reset}
  npm run release                         # Interactive release
  npm run release patch                   # Legacy patch release
...
async function main() {
  ...
  if (parseResult.legacy) {
    await legacyRelease(parseResult.versionType);
    return;
  }
